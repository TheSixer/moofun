 ...
 * 主页面
 * added by Lijie
 * @name haimaApp.controller:MainCtrl
 * @description
 * # MainCtrl
 * Controller of the haimaApp
 */
angular.module('haimaApp')
  .controller('MainCtrl', function($scope, $window, $location, $state, $rootScope, $uibModal, Util, HAIMA_END_POINT, Auth, userService, codeMasterService, localStorageService, MainApi) {
    ...
    var init = function() {
      ...
      // localStorage中存在否
      if (Util.isStored('haimaDmsIcon')) {
        // localStorage中取得icon信息
        $scope.iconMatrix = localStorageService.get('haimaDmsIcon');
      } else {
        // 取得图标信息
        $scope.iconPromise = MainApi.getIconMateData(function(result) {
          // 保存图标信息
          localStorageService.set('haimaDmsIcon', result.iconMatrix);
    ...
    var bindMenu = function(menuData) {
      ...
      for (var i in menuData) {
        if (menuData[i].text) {
          ...
          // }
          if ('MooFun管理系统'.equals(menuData[i].text)) {
            topMenu.push(menuData[i]);
          }
        }
      }
    ...
    var getMenuList = function() {
      ...
      // localStorage中存在否
      if (Util.isStored('haimaDmsMenu')) {

        var menuData = localStorageService.get('haimaDmsMenu');
        // 绑定到页面
        bindMenu(menuData);
      } else {

        $rootScope.menuPromise = MainApi.getMenu($scope.currentUser.username, function(result) {
          // 无数据
          if (!result.data) {
          ...
          // 保存
          localStorageService.set('haimaDmsMenu', result.data);
    ...
    $scope.openMenu = function(url, sMenu, tMenu, fMenu) {
      ...
      if ('/'.equals(path) || '/welcome'.equals(path)) {
