...
angular.module('moofunApp')
  .controller('BaseListCtrl', function($scope, $injector, CRUDServices, config) {
    ...
    // 每页的数据件数
    $scope.numPerPage = $scope.pageList[1];
    // 数据的总件数
    $scope.totalItems = 0;
    ...
    var formatstatistData = function(result, key) {
      ...
      for(var x = 0; x < statisticarr.length; x++) {
        for (var y = x + 1; y < statisticarr.length; y++) {
          var time1 = (new Date(statisticarr[x].date)).getTime();
          var time2 = (new Date(statisticarr[y].date)).getTime();
          
          if(time1 < time2) {
            var object = new Object(statisticarr[x]);
            statisticarr[x] = statisticarr[y];
            statisticarr[y] = object;
          }
        }
      }
    ...
    var formateDate = function(queryobj) {
      ...
      if (queryobj.hasOwnProperty('startTime')) {
        obj.startTime = moment(queryobj.startTime).format('YYYY-MM-DD');
      }

      if (queryobj.hasOwnProperty('endTime')) {
        obj.endTime = moment(queryobj.endTime).format('YYYY-MM-DD');
      }
    ...
    var loadData = function() {
      var param = {
        pageCount: $scope.numPerGet, //每页条数
        page: $scope.backendPage //页数
      };
      ...
      $scope[config.queryPromiseAlias] = CRUDServices[config.CRUDServicesMethodName](param, function(result) {
        ...
        formatData(result.data.list, param.page, param.pageCount);
