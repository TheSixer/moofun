...
angular.module('haimaApp')
  .factory('HybridUiGrid', function (PAGE_CONST,$injector) {
    ...
    return function () {
      ...
      //当前页数(前台)
      mHybridObj.fePageNum = 1;
      //当前页数(后台)
      mHybridObj.bePageNum = 1;
      //分页数(前台)
      ...
      mHybridObj.getDataFromFe = function () {
        ...
        var rowStart = (mHybridObj.fePageNum - 1) * mHybridObj.fePageSize - (mHybridObj.bePageNum - 1) * mHybridObj.bePageSize;
      ...
      mHybridObj.processBeforeGetDataFromBeCommon = function () {
        ...
        // 计算本次请求的后台页数
        mFilter.pageNum = mHybridObj.calcBePageNumBeforeGetDataFromBe();
      ...
      mHybridObj.processAfterGetDataFromBeCommon = function (rows, total) {
        ...
        mHybridObj.bePageNum = mHybridObj.calcBePageNumBeforeGetDataFromBe();
      ...
      mHybridObj.calcIdx = function (){
        ...
        var bePageNum = mHybridObj.bePageNum;
        var bePageSize = mHybridObj.bePageSize;

        for(var i = 0;i < mHybridObj.feData.length;i ++){
          mHybridObj.feData[i].idx = Util.getIdxForUiGrid(i + 1,bePageNum,bePageSize);
        }
      };
      ...
      mHybridObj.isOverFeRange = function () {
        ...
        if (!mHybridObj.feData || mHybridObj.feData.length === 0) {
          isOverFeRange = true;
        } else if (mHybridObj.fePageNum * mHybridObj.fePageSize > mHybridObj.bePageNum * mHybridObj.bePageSize) {
          isOverFeRange = true;
        } else if (mHybridObj.fePageNum * mHybridObj.fePageSize <= (mHybridObj.bePageNum - 1) * mHybridObj.bePageSize) {
          isOverFeRange = true;
        }
      ...
      //计算API端请求时的起始页数
      mHybridObj.calcBePageNumBeforeGetDataFromBe = function () {
        return Math.ceil(mHybridObj.fePageNum * mHybridObj.fePageSize / mHybridObj.bePageSize);
      };

      //换页时的处理
      mHybridObj.onPaginationChanged = function (newPage, pageSize) {
        mHybridObj.fePageNum = newPage;
      ...
      var calcGridHeight = function(){
        return mHybridObj.fePageNum * mHybridObj.fePageSize <= mHybridObj.gridOptions.totalItems ? mHybridObj.fePageSize : Math.floor(mHybridObj.gridOptions.totalItems % mHybridObj.fePageSize);
      };
